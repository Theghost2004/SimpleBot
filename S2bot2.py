        ...
        try:
            ...
            if not self.forwarding_enabled:
                ...
                if not command_name.startswith("/silent"):
                    ...
                    await event.reply("⚠️ --Ꮪ@'𝚜 𝙰𝙳𝙱𝙾𝚃 is currently offline! Use `/start` command to wake it up. 🚀")
                return None
...
class HumanBehaviorManager:
    ...
    def __init__(self):
        ...
        # Enhanced anti-detection delay ranges in seconds
        self.base_delay_ranges = {
            "message": (180, 360),       # Much longer delays (3-6 minutes)
            "response": (5, 25),         # Slower responses to commands
            "typing": (2, 8),            # Longer typing simulation
            "read": (3, 12),             # More realistic read delays
            "reaction": (4, 15),         # Longer reaction delays
            "consecutive": (300, 600),   # Very long delays between consecutive actions (5-10 min)
            "forward": (240, 480),       # Special forwarding delay (4-8 minutes)
            "join": (600, 1800),         # Long delays when joining channels (10-30 minutes)
            "bulk": (900, 2700)          # Extra long delays for bulk operations (15-45 minutes)
        }
        
        # Daily limits to prevent spam detection
        self.daily_limits = {
            "messages_sent": 0,
            "max_messages_per_day": 100,
            "forwards_sent": 0,
            "max_forwards_per_day": 50,
            "joins_today": 0,
            "max_joins_per_day": 3,
            "last_reset": datetime.now().date()
        }
        
        # Activity tracking for natural patterns
        self.activity_windows = {
            "active_hours": [(9, 12), (14, 17), (19, 22)],  # Natural active periods
            "weekend_reduction": 0.7,  # 30% less activity on weekends
            "night_reduction": 0.2     # 80% less activity at night
        }
    ...
    async def natural_delay(self, action_type="message", context=None):
        """Apply advanced anti-detection delay patterns"""
        # Check daily limits first
        if not self.check_daily_limits(action_type):
            logger.warning(f"Daily limit reached for {action_type}, skipping action")
            return False
            
        # Get base delay range for this action type
        base_min, base_max = self.base_delay_ranges.get(action_type, (30, 90))
        
        # Advanced consecutive action penalties
        if self.consecutive_count > 1 and self.last_action_type == action_type:
            # Exponential backoff for consecutive actions
            penalty_factor = 1.5 ** (self.consecutive_count - 1)
            base_min = min(base_min * penalty_factor, 1800)  # Cap at 30 minutes
            base_max = min(base_max * penalty_factor, 3600)  # Cap at 1 hour
        
        # Enhanced time-based behavior patterns
        hour_now = datetime.now().hour
        is_weekend = datetime.now().weekday() >= 5
        
        # Night hours (0-6): Very slow, minimal activity
        if 0 <= hour_now < 6:
            time_factor = random.uniform(3.0, 5.0)
            activity_chance = 0.1  # Only 10% chance to be active
        # Early morning (6-9): Gradually increasing activity
        elif 6 <= hour_now < 9:
            time_factor = random.uniform(1.5, 2.5)
            activity_chance = 0.4
        # Active hours (9-12, 14-17, 19-22): Normal activity
        elif self.is_active_window(hour_now):
            time_factor = random.uniform(0.8, 1.2)
            activity_chance = 0.9
        # Lunch break (12-14): Reduced activity
        elif 12 <= hour_now < 14:
            time_factor = random.uniform(1.8, 2.8)
            activity_chance = 0.3
        # Evening transition (17-19): Moderate activity
        elif 17 <= hour_now < 19:
            time_factor = random.uniform(1.2, 1.8)
            activity_chance = 0.6
        # Late night (22-24): Decreasing activity
        else:
            time_factor = random.uniform(2.0, 3.5)
            activity_chance = 0.2
            
        # Weekend factor
        if is_weekend:
            time_factor *= self.activity_windows["weekend_reduction"]
            activity_chance *= 0.7
            
        # Random skip based on activity chance
        if random.random() > activity_chance:
            skip_delay = random.uniform(1800, 7200)  # Skip for 30 minutes to 2 hours
            logger.info(f"Simulating natural inactivity - skipping for {skip_delay/60:.1f} minutes")
            await asyncio.sleep(skip_delay)
            return False
            
        # Apply time factor
        min_delay = base_min * time_factor
        max_delay = base_max * time_factor
        
        # Add randomness with multiple distribution patterns
        delay_patterns = [
            self._exponential_delay(min_delay, max_delay),
            self._gaussian_delay(min_delay, max_delay),
            self._weighted_random(min_delay, max_delay)
        ]
        delay = random.choice(delay_patterns)
        
        # Add micro-variations to avoid pattern detection
        micro_variation = random.uniform(0.9, 1.1)
        delay *= micro_variation
        
        # Final safety cap
        delay = min(delay, 7200)  # Maximum 2 hours
        
        logger.info(f"Advanced delay applied: {delay/60:.2f} minutes for {action_type}")
        await asyncio.sleep(delay)
        
        # Update daily counter
        self.update_daily_counter(action_type)
        return True
    ...
    def check_daily_limits(self, action_type):
        ...
        if self.daily_limits["last_reset"] != today:
            self.daily_limits.update({
                "messages_sent": 0,
                "forwards_sent": 0,
                "joins_today": 0,
                "last_reset": today
        ...
        if action_type in ["message", "response"] and self.daily_limits["messages_sent"] >= self.daily_limits["max_messages_per_day"]:
            return False
        elif action_type == "forward" and self.daily_limits["forwards_sent"] >= self.daily_limits["max_forwards_per_day"]:
            return False
        elif action_type == "join" and self.daily_limits["joins_today"] >= self.daily_limits["max_joins_per_day"]:
            return False
            
        return True
    
    def update_daily_counter(self, action_type):
        """Update daily action counters"""
        if action_type in ["message", "response"]:
            self.daily_limits["messages_sent"] += 1
        elif action_type == "forward":
            self.daily_limits["forwards_sent"] += 1
        elif action_type == "join":
            self.daily_limits["joins_today"] += 1
    
    def is_active_window(self, hour):
        """Check if current hour is in active windows"""
        for start, end in self.activity_windows["active_hours"]:
            if start <= hour < end:
                return True
        return False
    
    def _exponential_delay(self, min_val, max_val):
        """Generate exponentially distributed delay"""
        import math
        lambda_param = 2 / (min_val + max_val)
        delay = random.expovariate(lambda_param)
        return max(min_val, min(delay, max_val))
    
    def _gaussian_delay(self, min_val, max_val):
        """Generate normally distributed delay"""
    ...
    def should_take_break(self):
        """Determine if bot should take a longer break to simulate human patterns"""
        # Check recent activity density
        recent_activity = [a for a in self.recent_actions
                           if (datetime.now() - a["time"]).total_seconds() < 3600]  # Last hour
        
        # If very active recently, suggest a break
        if len(recent_activity) > 10:
            return True, random.uniform(1800, 5400)  # 30 minutes to 1.5 hours
            
        # Random breaks throughout the day
        if random.random() < 0.1:  # 10% chance
            return True, random.uniform(900, 3600)  # 15 minutes to 1 hour
        ...
        try:
            ...
            while campaign_id in self.active_monitors:
                ...
                if not campaign_data:
                    await asyncio.sleep(active_update_interval * 3)  # Reduce monitoring frequency
                    continue
                
                # Get current time for calculations
                current_time = time.time()
                
                # Auto-stop monitor after 2-3 days when ads are running
                campaign_start_time = campaign_data.get('start_time', time.time())
                days_running = (current_time - campaign_start_time) / 86400  # Convert to days
                
                if campaign_data.get('status') == 'running' and days_running >= 2:
                    logger.info(f"Auto-stopping monitor for campaign {campaign_id} after {days_running:.1f} days")
                    try:
                        await message.edit(f"📊 **Campaign Monitor Auto-Stopped**\n\n🕒 Campaign has been running for {days_running:.1f} days\n✅ Monitor automatically stopped to prevent resource usage\n\nUse `/monitor` to restart monitoring if needed.")
                ...
                await asyncio.sleep(update_interval * 2)  # Reduce monitoring frequency
...
class MessageForwarder:
    ...
    def __init__(self, client):
        self.client = client
        self.client.flood_sleep_threshold = 60  # Increase flood wait time for better safety
        self.flood_protection = {
            'max_messages': 100,  # Increase max messages for better performance
            'cooldown': 60,       # Keep cooldown period
        ...
        self.smart_mode = True  # Toggle for smart behavior - can be disabled for faster operation
    ...
    async def forward_stored_message(self, msg_id: str = "default", targets: Optional[Set[int]] = None, interval: Optional[int] = None, campaign_id: Optional[str] = None, max_queue_size: int = 100):
        ...
        try:
            ...
            while True:
                ...
                for i in range(0, len(target_list), batch_size):
                    ...
                    for target in batch:
                        try:
                            ...
                            # Apply advanced human-like delays to prevent account limits
                            if self.smart_mode:
                                # Log the action for behavior tracking
                                self.human_behavior.log_action("forward", target, {"type": "forward", "msg_id": msg_id})
                                
                                # Check if we should take a longer break
                                should_break, break_duration = self.human_behavior.should_take_break()
                                if should_break:
                                    logger.info(f"Taking extended break for {break_duration/60:.1f} minutes to simulate human behavior")
                                    await asyncio.sleep(break_duration)
                                
                                # Apply advanced natural delay for forwarding with anti-detection patterns
                                success = await self.human_behavior.natural_delay("forward", {"target": target, "batch_size": len(batch)})
                                if not success:
                                    logger.info(f"Skipping remaining targets in batch due to natural inactivity simulation")
                                    break  # Skip rest of batch during natural inactive periods
                        except Exception as e:
                            failure_count += 1
                            error_message = str(e)
                ...
                # Add enhanced delay between batches with human behavior patterns
                if last_batch_index + batch_size < len(target_list):
                    # Calculate intelligent delay based on batch performance and time of day
                    batch_delay_base = random.uniform(300, 900)  # 5-15 minutes base delay
                    
                    # Increase delay if there were failures in this batch
                    if failure_count > 0:
                        failure_penalty = min(failure_count * 60, 600)  # Up to 10 minutes extra
                        batch_delay_base += failure_penalty
                        logger.info(f"Increased batch delay by {failure_penalty/60:.1f} minutes due to {failure_count} failures")
                    
                    # Time-based adjustments for natural behavior
                    current_hour = datetime.now().hour
                    if 0 <= current_hour <= 6:  # Night time - much longer delays
                        batch_delay_base *= random.uniform(2.0, 4.0)
                    elif 22 <= current_hour <= 23:  # Late evening - longer delays
                        batch_delay_base *= random.uniform(1.5, 2.5)
                    elif current_hour in [12, 13, 18, 19]:  # Meal times - moderate delays
                        batch_delay_base *= random.uniform(1.2, 1.8)
                    
                    # Weekend increases
                    if datetime.now().weekday() >= 5:  # Weekend
                        batch_delay_base *= 1.3
                    
                    # Cap the delay at reasonable maximum
                    batch_delay = min(batch_delay_base, 3600)  # Maximum 1 hour between batches
                    
                    logger.info(f"Advanced batch delay: {batch_delay/60:.1f} minutes between batches to simulate natural behavior")
                ...
                # Enhanced round interval with dynamic adjustments for anti-detection
                enhanced_interval = use_interval
                
                # Increase interval based on failure rate to avoid triggering anti-spam
                if failure_count > 0:
                    failure_ratio = failure_count / (success_count + failure_count)
                    if failure_ratio > 0.3:  # More than 30% failures
                        enhanced_interval *= random.uniform(2.0, 4.0)  # Double to quadruple the interval
                        logger.warning(f"High failure rate ({failure_ratio:.1%}), extending interval significantly")
                    elif failure_ratio > 0.1:  # More than 10% failures
                        enhanced_interval *= random.uniform(1.5, 2.5)  # 1.5x to 2.5x interval
                        logger.info(f"Moderate failure rate ({failure_ratio:.1%}), extending interval")
                
                # Natural variation in timing (±20% randomness)
                enhanced_interval *= random.uniform(0.8, 1.2)
                
                # Add time-of-day multipliers for realistic human patterns
                current_hour = datetime.now().hour
                if 0 <= current_hour <= 6:  # Night: much longer intervals
                    enhanced_interval *= random.uniform(3.0, 6.0)
                    logger.info("Night time detected - using extended intervals")
                elif 22 <= current_hour <= 23:  # Late evening: longer intervals
                    enhanced_interval *= random.uniform(1.8, 2.5)
                elif current_hour in [7, 8, 12, 13, 18, 19, 20]:  # Busy hours: moderate extension
                    enhanced_interval *= random.uniform(1.2, 1.6)
                
                # Weekend behavior - generally slower
                if datetime.now().weekday() >= 5:
                    enhanced_interval *= random.uniform(1.3, 1.8)
                    logger.info("Weekend detected - using relaxed intervals")
                
                # Ensure minimum interval for safety (15 minutes minimum)
                enhanced_interval = max(enhanced_interval, 900)
                
                # Cap maximum interval (6 hours)
                enhanced_interval = min(enhanced_interval, 21600)
                
                logger.info(f"Enhanced anti-detection interval: {enhanced_interval/60:.1f} minutes before next round")
                await asyncio.sleep(enhanced_interval)
...
I am --Ꮪ@'𝚜 𝙰𝙳𝙱𝙾𝚃, your ultimate Telegram assistant, built to make your experience smarter, faster, and way more fun! 🎭⚡
...
"""
    ...
    async def cmd_stop(self, event):
        ...
        try:
            ...
            stop_message = f"""⚠️ --Ꮪ@'𝚜 𝙰𝙳𝙱𝙾𝚃 SYSTEM SHUTDOWN ⚠️
...
"""
    ...
    async def cmd_help(self, event):
        ...
        try:
            ...
            help_text = f"""🚀🔥 WELCOME TO --Ꮪ2'𝚜 𝙰𝙳𝙱𝙾𝚃 COMMAND CENTER 🔥🚀
...
🔥 Powered by --Ꮪ2'𝚜 𝙰𝙳𝙱𝙾𝚃 (@{username})
...
"""
    ...
    async def cmd_addtarget(self, event):
        """Add target chats including topics by serial number, chat ID, username, links, and UIDs"""
        try:
            command_parts = event.text.split()
            if len(command_parts) < 2:
...
• Usernames: @group
• User IDs: uid:123456789
• Multiple mixed: 1,@group,t.me/c/123/456,-100987654321,uid:123456789

Example: /addtarget @group1,t.me/c/123/456,-100123456789,uid:987654321"""
                await event.reply(usage)
                return
                ...
                try:
                    ...
                    if target.lower().startswith('uid:'):
                        try:
                            ...
                            chat_id = entity_id
                            chat_name = f"{entity_name} (uid:{uid})"
                        except Exception as e:
                            fail_list.append(f"{target}: Invalid user ID - {str(e)}")
                            continue

                    # Handle topic links (t.me/c/channelid/topicid)
                    elif 't.me/c/' in target or 'telegram.me/c/' in target:
                        ...
                        if topic_match:
                            ...
                            if topic_id:
                                ...
                                chat_name = f"Channel {chat_id[0]} (topic #{topic_id})"

                    # Handle regular t.me links and usernames
                    elif target.startswith('@') or 't.me/' in target or target.startswith('https://t.me/'):
                        try:
                            entity_id, entity_type, entity_name, resolved_topic = await resolve_entity_without_get_entity(self.client, target)
                            chat_id = entity_id
                            if resolved_topic:
                                chat_id = (entity_id, resolved_topic)
                                chat_name = f"{entity_name} (topic #{resolved_topic})"
                            else:
                                chat_name = f"{entity_name} (@{target.lstrip('@')})" if target.startswith('@') else entity_name
                        except Exception as e:
                            fail_list.append(f"{target}: Could not resolve - {str(e)}")
                            continue

                    # Try parsing as serial number if numeric and within range
                        ...
                        chat_name = f"Serial #{serial_no}"

                    # Try parsing as direct chat ID
                    elif target.lstrip('-').isdigit():
                        chat_id = int(target)
                        chat_name = f"ID: {chat_id}"

                    if chat_id:
                        # If no chat_name was set during resolution, try to get one
                        if not chat_name:
                            try:
                        ...
                        success_list.append(f"✅ {chat_name} → {chat_id}")
                        logger.info(f"Added target chat: {chat_id}")
                    else:
                        fail_list.append(f"{target}: Could not resolve target")
            ...
            result = f"📊 **Add Target Results:**\n\n"
            
            if success_list:
                result += f"**✅ Successfully Added ({len(success_list)}):**\n"
                for success in success_list[:10]:  # Limit to first 10
                    result += f"• {success}\n"
                if len(success_list) > 10:
                    result += f"... and {len(success_list) - 10} more\n"
            
            if fail_list:
                result += f"\n**❌ Failed ({len(fail_list)}):**\n"
                for failure in fail_list[:5]:  # Limit to first 5
                    result += f"• {failure}\n"
                if len(fail_list) > 5:
                    result += f"... and {len(fail_list) - 5} more\n"
            
            result += f"\n**📈 Total Targets Now: {len(self.target_chats)}**"
            await event.reply(result)
            
            logger.info(f"Added targets: {len(success_list)} success, {len(fail_list)} failed. Total targets: {len(self.target_chats)}")
        except Exception as e:
            logger.error(f"Error in addtarget command: {str(e)}")
            await event.reply(f"❌ Error: {str(e)}")

    @admin_only
    async def cmd_addtarget_old(self, event):
        """Legacy addtarget method - for reference only"""
        try:
    ...
    async def cmd_removetarget(self, event):
        """Remove target chats by serial number, chat ID, username, links, and UIDs"""
        try:
            command_parts = event.text.split()
            if len(command_parts) < 2:
                await event.reply("❌ Please provide targets to remove in any format:\n• Serial numbers: 1,2,3\n• Chat IDs: -100123456789\n• Usernames: @group\n• Links: t.me/group\n• User IDs: uid:123456789\n\nExample: /removetarget 1,@group,-100123456789,uid:987654321")
                return

            # Get list of targets for serial number lookup
                ...
                try:
                    ...
                    if target.lower().startswith('uid:'):
                        try:
                            uid = int(target[4:])
                            entity_id, entity_type, entity_name, _ = await resolve_entity_without_get_entity(self.client, uid)
                            chat_id_to_remove = entity_id
                        except Exception as e:
                            not_found.append(f"{target}: Invalid user ID - {str(e)}")
                            continue

                    # Try parsing as serial number first
                    elif target.isdigit() and int(target) > 0 and int(target) <= len(targets):
                        serial_no = int(target)
                        chat_id_to_remove = targets[serial_no - 1]  # Convert to 0-based index
                        
                    # Try parsing as direct chat ID
                    elif target.lstrip('-').isdigit():
                        chat_id_to_remove = int(target)
                        
                    # Handle username or link resolution
                    elif target.startswith('@') or 't.me/' in target or target.startswith('https://t.me/'):
                        try:
                            entity_id, entity_type, entity_name, resolved_topic = await resolve_entity_without_get_entity(self.client, target)
                            if resolved_topic:
                                chat_id_to_remove = (entity_id, resolved_topic)
                            else:
                                chat_id_to_remove = entity_id
                        except Exception as e:
                            not_found.append(f"{target}: Could not resolve - {str(e)}")
                            continue
                    else:
                        not_found.append(f"{target}: Invalid format")
                        continue

                    # Try to remove the target
                    if chat_id_to_remove and chat_id_to_remove in self.target_chats:
                        self.target_chats.remove(chat_id_to_remove)
                        # Try to get a friendly name
                        try:
                            if isinstance(chat_id_to_remove, tuple):
                                removed.append(f"Channel {chat_id_to_remove[0]} (topic #{chat_id_to_remove[1]})")
                            elif target.isdigit() and int(target) <= len(targets):
                                removed.append(f"Serial #{target} → {chat_id_to_remove}")
                            else:
                                removed.append(f"{target} → {chat_id_to_remove}")
                        except:
                            removed.append(str(chat_id_to_remove))
                    else:
                        not_found.append(f"{target}: Not found in targets")
                        
                except Exception as e:
                    logger.error(f"Error processing target {target}: {str(e)}")
                    not_found.append(f"{target}: Error - {str(e)}")
                    continue

            # Send comprehensive results
            result = f"📊 **Remove Target Results:**\n\n"
            
            if removed:
                result += f"**✅ Successfully Removed ({len(removed)}):**\n"
                for remove in removed:
                    result += f"• {remove}\n"
            
            if not_found:
                result += f"\n**❌ Not Found/Failed ({len(not_found)}):**\n"
            ...
            logger.info(f"Removed targets: {len(removed)} success, {len(not_found)} failed. Total targets: {len(self.target_chats)}")
        except Exception as e:
    ...
    async def cmd_joinchat(self, event):
        """Join chats using username, links, UIDs, and serial numbers with enhanced support"""
        try:
            chats = []
            
            # Get chats from command parameters or reply
            command_parts = event.text.split()
            if len(command_parts) > 1:
                # Parse from command parameters - supports comma-separated values
                chat_str = command_parts[1]
            ...
            if not chats:
                await event.reply("❌ Please provide chats to join in any format:\n• Usernames: @group\n• Links: t.me/group\n• User IDs: uid:123456789\n• Serial numbers: 1,2,3\n• Multiple: @group1,t.me/group2,uid:123456789,1\n\nExample: /joinchat @group1,t.me/group2,uid:987654321")
                return
            ...
            # Get available dialogs for serial number resolution
            all_chats = []
            async for dialog in self.client.iter_dialogs():
            ...
            for chat in chats:
                try:
                    ...
                    # Handle user ID format (uid:12345)
                    if chat.lower().startswith('uid:'):
                        try:
                            uid = int(chat[4:])
                            entity_id, entity_type, entity_name, _ = await resolve_entity_without_get_entity(self.client, uid)
                            resolved_chat = entity_name if entity_name else f"@user{uid}"
                        except Exception as e:
                            fail_list.append(f"• {chat}: Invalid user ID - {str(e)}")
                            continue
                    
                    # Handle serial numbers
                    elif chat.isdigit() and int(chat) > 0 and int(chat) <= len(all_chats):
                        serial_no = int(chat)
                        chat_id = all_chats[serial_no - 1]
                        try:
                            entity_id, entity_type, entity_name, _ = await resolve_entity_without_get_entity(self.client, chat_id)
                            resolved_chat = entity_name if entity_name else str(chat_id)
                        except:
                            resolved_chat = str(chat_id)
                    
                    # Handle direct chat IDs
                    elif chat.lstrip('-').isdigit():
                        resolved_chat = int(chat)
                    
                    # Use chat as is for usernames and links
                    else:
                        resolved_chat = chat
                    
                    if resolved_chat:
                        success, result = await self._join_with_delay(resolved_chat)
                        if success:
                            success_list.append(f"• {chat} → Joined successfully")
                        else:
                        ...
                        fail_list.append(f"• {chat}: Could not resolve")
...
"""
    ...
    async def cmd_remove_failed(self, event):
        """Remove failed chats from targets and leave those chats"""
        try:
            # Parse command arguments
            args = event.text.split()[1:] if len(event.text.split()) > 1 else []
            ...
            if len(remove_chats) > 5:
                preview += f"... and {len(remove_chats) - 5} more chats\n"
            
            await msg.edit(f"🚀 **Starting Enhanced Removal Operation**\n\nRemoving {len(remove_chats)} chats from failed list and targets, plus leaving those chats...{preview}")
            
            # Enhanced removal operation
            removed_from_failed = 0
            removed_from_targets = 0
            left_chats = 0
            leave_errors = []
            
            for i, (chat_id, data) in enumerate(remove_chats.items(), 1):
                try:
                    # Convert chat_id string back to int if needed
                    chat_id_int = int(chat_id) if isinstance(chat_id, str) and chat_id.lstrip('-').isdigit() else chat_id
                    
                    # Remove from failed chats list
                    if chat_id in self.failed_chats:
                        del self.failed_chats[chat_id]
                        removed_from_failed += 1
                    
                    # Remove from target chats if present
                    if chat_id_int in self.target_chats:
                        self.target_chats.remove(chat_id_int)
                        removed_from_targets += 1
                    
                    # Try to leave the chat
                    try:
                        await self.client.leave_channel(chat_id_int)
                        left_chats += 1
                        logger.info(f"Left chat: {chat_id_int}")
                    except Exception as leave_error:
                        leave_errors.append(f"Chat {chat_id}: {str(leave_error)[:50]}...")
                        logger.error(f"Failed to leave chat {chat_id_int}: {str(leave_error)}")
                    
                    # Progress update every 10 chats
                    if i % 10 == 0:
                        await msg.edit(f"🔄 **Processing Removal** ({i}/{len(remove_chats)})\n\nRemoved from failed: {removed_from_failed}\nRemoved from targets: {removed_from_targets}\nLeft chats: {left_chats}")
            ...
            result = f"✅ **Enhanced Removal Complete!**\n\n"
            result += f"📊 **Summary:**\n"
            result += f"• Removed from failed list: {removed_from_failed}\n"
            result += f"• Removed from targets: {removed_from_targets}\n"
            result += f"• Left chats: {left_chats}\n"
            
            if leave_errors:
                result += f"\n⚠️ **Leave Errors ({len(leave_errors)}):**\n"
                for error in leave_errors[:3]:  # Show first 3 errors
                    result += f"• {error}\n"
                if len(leave_errors) > 3:
                    result += f"... and {len(leave_errors) - 3} more errors\n"
            
            result += f"\n📈 **Current Status:**\n"
            result += f"• Failed chats remaining: {len(self.failed_chats)}\n"
            result += f"• Target chats remaining: {len(self.target_chats)}\n"
            
            await msg.edit(result)
            logger.info(f"Enhanced removal complete: {removed_from_failed} from failed, {removed_from_targets} from targets, {left_chats} left chats")
...
async def main_with_retry():
    """Main function without retry mechanism to prevent restart loops"""
    try:
        # Run the main bot function once only
        exit_code = await main()
        logger.info(f"Bot exited with code {exit_code}")
        return exit_code
    except Exception as e:
        logger.error(f"Exception in main_with_retry: {str(e)}")
        return 1
        ...
        async def keep_alive():
            ...
            try:
                ...
                try:
                    ...
                    startup_message = f"""S2BOT IS READY TO RUN"""
            ...
            while True:
                try:
                    ...
                    if ping_count % 30 == 0:
                        ...
                        if ping_count % 10 == 0:  # Only check every 10 minutes instead of 1 minute
                            # Very light connection check without API calls
                            if client.is_connected():
                                logger.info(f"Bot still running normally - heartbeat #{ping_count // 10}")
                                consecutive_failures = 0
                                reconnect_delay = 5
                            else:
                                logger.warning("Client disconnected, attempting to reconnect...")
                                await client.connect()
                        # Remove the debug heartbeat - too much logging
